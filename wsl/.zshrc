...
# setopt correct         # disabled: [nyae] prompt eats keystrokes, felt like broken input
...
# setopt noclobber       # disabled: breaks scripts that assume '>' overwrites
...
if ! socat -u OPEN:/dev/null UNIX-CLIENT:$SSH_AUTH_SOCK >/dev/null 2>&1; then
  rm -f $SSH_AUTH_SOCK
  (setsid socat UNIX-LISTEN:$SSH_AUTH_SOCK,fork EXEC:"/mnt/c/Users/Marco/AppData/Local/Microsoft/WinGet/Packages/albertony.npiperelay_Microsoft.Winget.Source_8wekyb3d8bbwe/npiperelay.exe -ei -s //./pipe/openssh-ssh-agent",nofork &) >/dev/null 2>&1
fi
